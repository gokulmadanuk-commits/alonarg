...
def decide(event: dict | None, recording: bool, active_key: str | None) -> str:
    """Pure decision for the scheduler: ``"start"``, ``"stop"`` or ``"none"``.

    - ``start`` when not recording and an approved event is in progress.
    ...
    if not recording and event is not None and is_approved(event):
        return "start"
